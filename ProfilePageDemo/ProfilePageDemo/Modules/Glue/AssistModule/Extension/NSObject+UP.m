...
- (UIImage *)imageByBlurRadius:(CGFloat)blurRadius {
    return [self imageByBlurRadius:blurRadius tintColor:nil tintMode:kCGBlendModeNormal saturation:1 maskImage:nil];
}
...
- (UIImage *)snapshotImageAfterScreenUpdates:(BOOL)afterUpdates {
    return [self snapshotImageAfterScreenUpdates:afterUpdates cropRect:self.bounds];
}

- (UIImage *)snapshotImageAfterScreenUpdates:(BOOL)afterUpdates cropRect:(CGRect)rect {
    if (![self respondsToSelector:@selector(drawViewHierarchyInRect:afterScreenUpdates:)]) {
        return [self snapshotImage];
    }
    UIGraphicsBeginImageContextWithOptions(rect.size, self.opaque, 0);
    [self drawViewHierarchyInRect:CGRectMake(-rect.origin.x, -rect.origin.y, self.frame.size.width, self.frame.size.height) afterScreenUpdates:afterUpdates];
    UIImage *snap = UIGraphicsGetImageFromCurrentImageContext();
    UIGraphicsEndImageContext();
    return snap;
}
