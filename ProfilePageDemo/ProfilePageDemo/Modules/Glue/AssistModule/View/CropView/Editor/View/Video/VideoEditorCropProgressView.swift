...
final class VideoEditorCropProgressView: UIView {
    ...
    private(set) var leftSpace: CGFloat = 0
    private(set) var rightSpace: CGFloat = 1
    ...
    private func layout(updateProgress: Bool) {
        let isSelected = rightSpace - leftSpace != 1
        ...
        contentView.snp.updateConstraints { maker in
            maker.left.equalToSuperview().offset(leftSpace*bounds.width)
            maker.right.equalToSuperview().offset(-((1-rightSpace)*(bounds.width)))
        }
        if updateProgress {
...
extension VideoEditorCropProgressView {
    ...
    public func setProgress(_ progress: CGFloat) {
        ...
        progress = progress < leftSpace ? leftSpace : (progress > rightSpace ? rightSpace : progress)
        let offset = (progress - leftSpace) / (rightSpace - leftSpace) * (progressContentView.frame.width - progressView.frame.width)
        progressView.snp.updateConstraints { maker in
...
extension VideoEditorCropProgressView {
    ...
    @objc private func progressViewPan(_ pan: UIPanGestureRecognizer) {
        ...
        if progress < leftSpace || progress > rightSpace {
            return
        }
    ...
    @objc private func leftButtonPan(_ pan: UIPanGestureRecognizer) {
        ...
        if rightSpace - tmpLeft < 0.2 {
            return
        }
        leftSpace = tmpLeft
        setProgress(leftSpace)
        layout(updateProgress: false)
        delegate?.cropProgress(self, didUpdate: leftSpace)
        setTimeline(hidden: pan.state != .changed)
    }
    ...
    @objc private func rightButtonPan(_ pan: UIPanGestureRecognizer) {
        ...
        setProgress(right)
        layout(updateProgress: false)
        delegate?.cropProgress(self, didUpdate: rightSpace)
        setTimeline(hidden: pan.state != .changed)
