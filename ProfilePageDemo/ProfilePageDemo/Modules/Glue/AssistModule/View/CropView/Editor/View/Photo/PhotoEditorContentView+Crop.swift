...
extension PhotoEditorContentView {
    ...
    func setCrop(_ option: EditorCropOption) {
        cropOption = option
        cropContext.cropIndex = self.options.cropOptions.firstIndex(of: option) ?? 0
...
extension PhotoEditorContentView {
    ...
    func layoutEndCrop(_ fromCache: Bool = false) {
        if fromCache {
            ...
            lastCropData.cropOptionIdx = cropContext.cropIndex
        ...
        UIView.animate(withDuration: fromCache ? 0 : 0.25) {
            ...
            if self.presetFitFrame != .zero {
                self.scrollView.center = self.presetFitFrame.center
            }
        }
...
extension PhotoEditorContentView {
    ...
    fileprivate var cropOption: EditorCropOption {
        ...
        set {
            cropContext.cropOption = newValue }
    }
    /// 上次裁剪开始时图片的Bounds
